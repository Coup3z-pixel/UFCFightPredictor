...
def processing_data():
    ...
    fighter_details = pd.read_csv('../dataset/fighter_details.csv')
    ...
    fighter_details.set_index('name', inplace=True)
    ...
    merged_fighter_df = fighter_details.merge(fighter_df, how='inner', on='name')
    ...
    try:
        ...
        for index, row in enumerate(fight_df.itertuples()):
            ...
            print(row[6] == "W")
            print(row[7])
            print(merged_fighter_df.loc[row[7]])
            """

            result_arr = [row[6] == "W"]

            try:
                r_fighter = merged_fighter_df.loc[row[7]].to_numpy().flatten()
                b_fighter = merged_fighter_df.loc[row[8]].to_numpy().flatten()

                train_df.loc[index] = np.hstack([
                    result_arr,
                    r_fighter,
                    b_fighter
                ])
